...
/*		var departamento = $('#departamento').val();
	...
	$( "#indicadora1" ).autocomplete({
	    minLength: 1,
	    source: window.location.protocol + "//" + window.location.host + "/vih-sida/index.php/fichas/autollenadoDiagno",
	    select: function(event, ui) {
	        $( "#ciex1" ).val( ui.item.id);
	        $( "#indicadora1" ).val( ui.item.value);
	    }
	});
	$( "#indicadora1" ).change(function(){
		if($( "#indicadora1" ).val()==''){
			$( "#ciex1" ).val('');
		};
	});

	$( "#indicadora2" ).autocomplete({
	    minLength: 1,
	    source: window.location.protocol + "//" + window.location.host + "/vih-sida/index.php/fichas/autollenadoDiagno",
	    select: function(event, ui) {
	        $( "#ciex2" ).val( ui.item.id);
	        $( "#indicadora2" ).val( ui.item.value);
	    }
	});
	$( "#indicadora2" ).change(function(){
		if($( "#indicadora2" ).val()==''){
			$( "#ciex2" ).val('');
		};
	});
...
function activarCheckbox0(source){
	if(document.getElementById('nino_vih').checked == true){
		document.getElementById('estadio').value = "";
		document.getElementById('estadio').disabled = true;
	}else{
		document.getElementById('estadio').disabled = false;
	}	
}

function activarCheckbox1(source){
...
function activarUbigeo(){
	if(document.getElementById('paises').value != "171"){
...
function exportandoms(){
	...
	$('#exportandoms').text('Exportando... espere la ventana de descarga un momento por favor, puede tardar varios segundos.');
	setTimeout(function(){ $("#exportandoms").fadeOut(800).fadeIn(800).fadeOut(500).fadeIn(500).fadeOut(300);}, 5000);
}