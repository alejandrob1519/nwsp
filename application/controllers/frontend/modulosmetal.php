...
class Modulosmetal extends CI_Controller {

    public function __construct()
    {
		parent::__construct();
        $this->layout->setLayout('template4metal');
        $this->layout->setTitle(":: NotiWeb :: Intoxicaci&oacute;n por Metales Pesados");
    }
	...
	//Grocery Crud: Listado de Fichas del Módulo Metales
    public function listarMetales()
    {
		...
		$crud->add_action('Modificar Ficha', '', 'modulosmetal/modificarMetales','edit-icon');
		$crud->add_action('Ver Ficha', '', 'modulosmetal/verMetales','read-icon');
		$output = $crud->render();

		$this->_example_output($output);
    }

    public function listarFactorMetal()
    {
		$eess = $this->fichas_model->mostrarEstablecimiento();
		...
		$crud->add_action('Modificar Ficha', '', 'modulosmetal/modificarFactorMetal','edit-icon');
		$crud->add_action('Ver Ficha', '', 'modulosmetal/verFactorMetales','read-icon');
		$output = $crud->render();

		$this->_example_output3($output);
    }
	
	//Grocery Crud: Listado de casos de intoxicacion por metales pesados notificados
    public function listarCasos()
    {
		...
		/*$filtro = array("substr(diagnostic,1,3)"=>"T56");*/
		$filtro = "substr(diagnostic,1,3)='T56' or substr(diagnostic,1,3)='T57'";
		$crud = new grocery_CRUD();
		...
		$crud->add_action('Registrar Ficha', '', 'modulosmetal/registrarMetales','edit-icon');
		$output = $crud->render();

		$this->_example_output1($output);
    }

	//Grocery Crud: Listado de fichas por intoxicacion de metales pesados
    public function listarBrotemetal()
    {
		$eess = $this->fichas_model->mostrarEstablecimiento();
		...
		$crud->add_action_peru('', '', 'registrarBrotemetal','add-icon');
		$crud->add_action('Registrar Intoxicados', '', 'modulosmetal/listarGrillaBrotemetal','more-icon');
		$crud->add_action('Registrar Ficha', '', 'modulosmetal/modificarBrotemetal','edit-icon');
		$output = $crud->render();

		$this->_example_output2($output);
    }

	//Grocery Crud: Listado de fichas de brotes de metales pesados
    public function listarGrillaBrotemetal($id=null)
    {
		$where = array('codigo'=>$id);
		
		$crud = new grocery_CRUD();

		$crud->set_table('gmetales');
		$crud->where($where);
	...
	//Registra una nueva ficha de metales pesados
    public function registrarMetales($id=null)
    {
		if(!$id)
		{
			show_404();
        ...
        if($this->input->post()){
			...
			if ($this->form_validation->run("modulosmetal/metales")){
			
				$numeroFicha = $this->metal_model->numeroMetales($this->input->post("captado", true));
				
				foreach($numeroFicha as $dato)
				{
					$numero=$dato->numero;
				}
				...
				(
					...
					"tiempo_act"=>$this->input->post("tiempo_act", true),
					...
					"tie_res_act"=>$this->input->post("tie_res_act", true),
					"tie_res_ant"=>$this->input->post("tie_res_ant", true),
					...
					"procedencia1"=>$this->input->post("procedencia1", true),
					"acteco"=>$this->input->post("acteco", true),
        ...
        $this->layout->view("registrarMetales", compact("id", "datos"));
    }

	//Modificar ficha de metales pesados
    public function modificarMetales($id=null)
    {
		if(!$id)
		{
			show_404();
        ...
        if($this->input->post()){
			...
			if ($this->form_validation->run("modulosmetal/metales")){
				...
				$guardar = $this->fichas_model->modificarMetales($data, $id);
				
				if($guardar)
				{
					$this->session->set_flashdata('exito', 'Informaci&oacute;n guardada con &eacute;xito.');
					redirect(site_url('modulosmetal/listarMetales'), 301);
				}else{
					$this->session->set_flashdata('error', 'No se guard&oacute; la informaci&oacute;n registrada.');
					redirect(site_url('modificarMetales'), 301);
				}
			}
        }

		$plaga = $this->fichas_model->buscarMetales($id);
        ...
        $this->layout->view("modificarMetales", compact("id", "plaga", "datos", 'departamento', 'provincia', 'distrito', 'departamento14_1', 'provincia14_1', 'distrito14_1'));
    }

	//Ver ficha de metales
    public function verMetales($id=null)
    {
		if(!$id)
		{
			show_404();
        ...
        $this->layout->view("verMetales", compact("id", "plaga", "datos", 'departamento', 'provincia', 'distrito', 'departamento14_1', 'provincia14_1', 'distrito14_1'));
    }

// richard
    public function registrarFactorMetal($id=null)
    {
		if(!$id)
		{
			show_404();
        ...
        if($this->input->post()){
			...
			if ($this->form_validation->run("modulosmetal/factormetal")){
				$data = array
				(
					"codigo"=>$this->input->post("codigo", true),
					"fecha_loc"=>$this->fechas_model->arreglarFechas($this->input->post("fecha_loc", true)),
				...
				$guardar = $this->fichas_model->insertarFactorMetal($data);
				...
				if($guardar)
				{
					...
					$this->session->set_flashdata('error', 'No se guard&oacute; la informaci&oacute;n registrada.');
					redirect(site_url('registrarFactorMetal'), 301);
				}
			}
        }
        ...
        $this->layout->view("registrarFactorMetal", compact("id", "datos"));
    }

	//Modificar ficha de metales pesados
    public function modificarFactorMetal($id=null)
    {
		if(!$id)
		{
			show_404();
        ...
        if($this->input->post()){
			...
			if ($this->form_validation->run("modulosmetal/factormetal")){
				...
				$guardar = $this->fichas_model->modificarFactorMetal($data, $id);
				
				if($guardar)
				{
					$this->session->set_flashdata('exito', 'Informaci&oacute;n guardada con &eacute;xito.');
					redirect(site_url('modulosmetal/listarFactorMetal'), 301);
				}else{
					$this->session->set_flashdata('error', 'No se guard&oacute; la informaci&oacute;n registrada.');
					redirect(site_url('modificarFactorMetal'), 301);
				}
			}
        }

		$plaga = $this->fichas_model->buscarFactorMetal($id);
        ...
        $this->layout->view("modificarFactorMetal", compact("id", "plaga", "datos", 'departamento', 'provincia', 'distrito', 'departamento14_1', 'provincia14_1', 'distrito14_1'));
    }

	//Ver ficha de metales
    public function verFactorMetal($id=null)
    {
		if(!$id)
		{
			show_404();
        ...
        $this->layout->view("verFactorMetal", compact("id", "plaga", "datos", 'departamento', 'provincia', 'distrito', 'departamento14_1', 'provincia14_1', 'distrito14_1'));
    }

// richard
	//Registra una nueva ficha de contingencia por metales pesados
    public function registrarBrotemetal()
    {
        ...
        if($this->input->post()){
			...
			if ($this->form_validation->run("modulosmetal/brotemetal")){
				$data = array
				(
					"codigo"=>$this->input->post("codigo", true),
					"fecha_loc"=>$this->fechas_model->arreglarFechas($this->input->post("fecha_loc", true)),
				...
				$guardar = $this->fichas_model->insertarBrotemetal($data);
				
				if($guardar)
				{
					$this->session->set_flashdata('exito', 'Informaci&oacute;n guardada con &eacute;xito.');
					redirect(site_url('modulosmetal/listarBrotemetal'), 301);
				}else{
					$this->session->set_flashdata('error', 'No se guard&oacute; la informaci&oacute;n registrada.');
		...
		if($this->session->userdata('sesionIniciada') == TRUE){
			$this->layout->view("registrarBrotemetal", compact('session_id', 'diresa', 'redes', 'microred', 'establec', 'departamento', 'provincia', 'distrito', 'departamento14_1', 'provincia14_1', 'distrito14_1'));
        }else{
            redirect(site_url("index/login"), 301);
        }
    }

	//Modifica una nueva ficha Situacion de Contingencia por Exposicion Metales pesados
    public function modificarBrotemetal($id=null)
    {
        ...
        if($this->input->post()){
			...
			if ($this->form_validation->run("modulosmetal/brotemetal")){
				...
				$guardar = $this->fichas_model->modificarBrotemetal($data, $id);
				
				if($guardar)
				{
					$this->session->set_flashdata('exito', 'Informaci&oacute;n guardada con &eacute;xito.');
					redirect(site_url('modulosmetal/listarBrotesmetal'), 301);
				}else{
					$this->session->set_flashdata('error', 'No se guard&oacute; la informaci&oacute;n registrada.');
					redirect(site_url('registrarBrotemetal'), 301);
				}
			}
        }
		
		$datos = $this->fichas_model->mostrarBrotemetal($id);
		...
		if($this->session->userdata('sesionIniciada') == TRUE){
			$this->layout->view("modificarBrotemetal", compact('session_id', 'datos', 'diresa', 'redes', 'microred', 'establec', 'departamento', 'provincia', 'distrito', 'departamento14_1', 'provincia14_1', 'distrito14_1'));
        }else{
            redirect(site_url("index/login"), 301);
        }
    }

	//Exporta la base de datos de la ficha de Metales Pesados
	public function exportarMetales()
	{
	...
	function validar_fecha($str){
	   ...
	   //buscar numero de metales Pesados
	public function numMetales()
	{
		$filtro = $this->input->post('captado');
		$numero = $this->metal_model->numeroMetales($filtro);
		$data = array();
		
		foreach($numero as $dato)
		{
