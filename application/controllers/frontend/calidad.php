...
class Calidad extends CI_Controller {
    ...
    {
        parent::__construct();
        $this->layout->setLayout('templateCalidad');
	...
	public function duplicados(){
		...
		redirect(site_url('calidad/individualDuplicados'), 301);
	}
	...
	public function inconsistencias(){
		...
		redirect(site_url('calidad/individualInconsistencias'), 301);
	}
	...
	public function individualInconsistencias(){
		...
		$crud->add_action('Editar', '', 'calidad/modIndividual','edit-icon');
		$crud->callback_after_delete(array($this,'log_registro_after_delete'));
	...
	public function nombres(){
		...
		redirect(site_url('calidad/individualNombres'), 301);
	}
	...
	public function edasDefunciones(){
		...
		$crud->add_action('Editar', '', 'calidad/modEdas','edit-icon');
    ...
    public function modEdas($id=null)
    {
        ...
        if($this->input->post()){
            if ($this->form_validation->run("edas/edas"))
            {
				...
				if($cantidad == 0){
					...
					redirect("calidad/edasDefunciones", 301);
				}
		...
		if(!empty($this->session_id)){
			if($grabar_id != '1' or $modificar_id != '1'){
				$this->session->set_flashdata('error', 'Error: Usted no cuenta con autorizaci&oacute;n para grabar &oacute; modificar registros.');
	            redirect(site_url("calidad/edasDefunciones"), 301);
			}
	        $this->layout->setLayout('template4');
    ...
    public function modIras($id=null)
    {
        ...
        if($this->input->post()){
            if ($this->form_validation->run("iras/iras"))
            {
				...
				if($cantidad == 0){
					$this->session->set_flashdata('error', 'Debe ingresar alguna atenci&oacute;n. NO se grab&oacute; el registro');
					redirect("calidad/irasDefunciones", 301);
				}
		...
		if(!empty($this->session_id)){
			if($grabar_id != '1' or $modificar_id != '1'){
				$this->session->set_flashdata('error', 'Error: Usted no cuenta con autorizaci&oacute;n para grabar &oacute; modificar registros.');
	            redirect(site_url("calidad/irasDefunciones"), 301);
			}
